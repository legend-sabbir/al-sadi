import{arr}from"./refrigeratorData.js";import{deepArr}from"./refrigeratorData.js";import{beverageArr}from"./refrigeratorData.js";const Head=document.querySelector("[data-head]"),title=document.querySelector("title");let url=window.location.href,product=url.split("?")[1];const imgFolder=(product=(title.innerHTML=product).includes("#")?product.split("#")[0]:product).split("-")[0];let img1Src,img2Src,img3Src,img4Src,img5Src;async function carousel(){const imgArr=[img1Src,img2Src,img3Src,img4Src,img5Src];imgArr.forEach((img,i)=>{fetch(img).then(res=>{if(!res.ok){const navContainer=document.querySelector(".navigation"),slideContainer=document.querySelector(".slider-container");var res=document.querySelector(".slide-"+(i+1)),navImg=document.querySelector(".navigation .img-"+(i+1));slideContainer.removeChild(res),navContainer.removeChild(navImg),--len,marker.style.transform=`translateX(${navigationImg[0].offsetLeft}px)`}})}),fetch(imgArr[1]).then(res=>{const navContainer=document.querySelector(".navigation");res.ok?autoPlay():navContainer.remove()});var carouselData=`<section class="carousel">
...
`;const carouselSection=document.querySelector(".carousel");carouselSection.innerHTML=carouselData}[...arr,...deepArr,...beverageArr].forEach((item,index)=>{if(item.name===product){img1Src=`/refrigerator/img/${imgFolder}/${item.name}/i1.webp`,img2Src=`/refrigerator/img/${imgFolder}/${item.name}/i2.webp`,img3Src=`/refrigerator/img/${imgFolder}/${item.name}/i3.webp`,img4Src=`/refrigerator/img/${imgFolder}/${item.name}/i4.webp`,img5Src=`/refrigerator/img/${imgFolder}/${item.name}/i5.webp`;var minusLower=item.name.toLowerCase().replace(/-/g," "),minus=item.name.replace(/-/g," "),minus=(Head.content=`${item.name}, ${item.name.toLowerCase()}, ${minus}, `+minusLower,`   <div class="product-info">
